...
StatementMatcher ConfigSection = memberExpr(hasObjectExpression(hasType( cxxRecordDecl(isSameOrDerivedFrom("Spooky::Config::ConfigFile::Section"))))).bind("cfgSection");
StatementMatcher ConfigClass = memberExpr(hasObjectExpression(hasType( cxxRecordDecl(isSameOrDerivedFrom("Spooky::Config::ConfigFile"))))).bind("cfgClass");
...
DeclarationMatcher LocalAlarmMatcher= varDecl(hasType(asString("Spooky::Factory::AlarmClient")),
        ...
        anyOf(hasDescendant(ConfigClass), anything()),
        anyOf(hasDescendant(ConfigSection), anything()),
        anyOf(hasDescendant(stringLiteral().bind("strLit")), anything())
        ).bind("inst");
...
StatementMatcher PtrAlarmMatcher= cxxOperatorCallExpr(
        hasOverloadedOperatorName("="),
        hasDescendant(memberExpr(hasType(asString("std::unique_ptr<Spooky::Factory::AlarmClient>"))).bind("lhs")),
        anyOf(hasDescendant(ConfigClass), anything()),
        anyOf(hasDescendant(ConfigSection), anything()),
        anyOf(hasDescendant(stringLiteral().bind("strLit")), anything())
        ).bind("ptrAssign");
class PtrAlarmHandler: public MatchFinder::MatchCallback
{
public:
  virtual void run(const MatchFinder::MatchResult &Result)
  {
     auto *assignment = Result.Nodes.getNodeAs<clang::CXXOperatorCallExpr>("ptrAssign");
     auto *lhs = Result.Nodes.getNodeAs<clang::MemberExpr>("lhs");
    auto alarmInfo = readConfigArg(Result);
    if (!assignment || !lhs) return;
    auto lhsName = lhs->getMemberDecl()->getQualifiedNameAsString();
    coutInstantiatedJson(lhsName, alarmInfo);
  }

};

auto CtorAlarmMatcher= cxxConstructorDecl(
        forEachConstructorInitializer(
            cxxCtorInitializer(
                forField(
                    hasType(asString("std::unique_ptr<Spooky::Factory::AlarmClient>"))
                ),
                withInitializer(
                    allOf(
                        anyOf(hasDescendant(ConfigClass), anything()),
...
class CtorAlarmHandler: public MatchFinder::MatchCallback
{
public:
  virtual void run(const MatchFinder::MatchResult &Result)
  {
     auto *memberInit = Result.Nodes.getNodeAs<clang::CXXCtorInitializer>("memberInit");
    auto alarmInfo = readConfigArg(Result);
    if(alarmInfo.first == InstantiationResultType::Error) return;
    if (!memberInit) return;
    auto className = memberInit->getMember()->getParent()->getQualifiedNameAsString();
    auto fieldName= memberInit->getMember()->getNameAsString();
    coutInstantiatedJson(className + "::" + fieldName, alarmInfo);
  }
...
int main(int argc, const char **argv)
{
  ...
  PtrAlarmHandler ptrAlarmHandler;
  finder.addMatcher(PtrAlarmMatcher, &ptrAlarmHandler);

  CtorAlarmHandler ctorAlarmHandler;
  finder.addMatcher(CtorAlarmMatcher, &ctorAlarmHandler);
